...
async function ensureTable() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS staff_verifications (
      ...
      CONSTRAINT staff_verifications_status_check CHECK (status IN ('pending','submitted','approved','rejected'))
    )
  `);
}
...
publicRouter.get("/:token", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      "SELECT status FROM staff_verifications WHERE token_hash=$1 LIMIT 1",
      [tokenHash(token)],
    );
  ...
  async (req, res) => {
    try {
      ...
      const found = await pool.query(
        "SELECT id,status FROM staff_verifications WHERE token_hash=$1 LIMIT 1",
        [tokenHash(token)],
      );
      ...
      const profile: ProfileData = {
        ...
        emergencyContactName: safeText(req.body.emergencyContactName, 160),
        emergencyContactPhone: safeText(req.body.emergencyContactPhone, 80),
      };
      ...
      await pool.query(
        `UPDATE staff_verifications SET
...
adminRouter.post("/staff/:staffId/verification-link", async (req, res) => {
  try {
    await ensureTable();
    const staffId = parseIdParam(req, res, "staffId");
    if (!staffId) { res.status(400).json({ error: "Invalid client id" }); return; }
    const client = await pool.query("SELECT id FROM admin_staff WHERE id=$1", [staffId]);
    if (!client.rows[0]) { res.status(404).json({ error: "Staff account not found" }); return; }
    const current = await pool.query("SELECT status FROM staff_verifications WHERE staff_id=$1", [staffId]);
    if (["submitted", "approved"].includes(current.rows[0]?.status)) {
      res.status(409).json({ error: current.rows[0].status === "submitted" ? "Verification is awaiting owner review" : "Staff verification is already approved" });
      return;
    }
    const token = crypto.randomBytes(32).toString("base64url");
    await pool.query(
      `INSERT INTO staff_verifications(staff_id,token_hash,status)
       VALUES($1,$2,'pending')
       ON CONFLICT(staff_id) DO UPDATE SET
         token_hash=EXCLUDED.token_hash,status='pending',reviewed_at=NULL,owner_note=NULL,updated_at=NOW()`,
      [staffId, tokenHash(token)],
    );
    res.setHeader("Cache-Control", "no-store, private");
    res.json({ path: `/staff-verification/${token}`, status: "pending" });
  } catch (error) {
    req.log.error(error);
...
adminRouter.get("/staff/:staffId/verification", async (req, res) => {
  try {
    await ensureTable();
    const staffId = parseIdParam(req, res, "staffId");
    if (!staffId) { res.status(400).json({ error: "Invalid client id" }); return; }
    const result = await pool.query(
      `SELECT id,status,profile_blob,selfie_blob,id_front_blob,id_back_blob,
       ...
       FROM staff_verifications WHERE staff_id=$1 LIMIT 1`,
      [staffId],
    );
...
adminRouter.get("/staff/:staffId/verification/image/:kind", async (req, res) => {
  try {
    await ensureTable();
    const staffId = parseIdParam(req, res, "staffId");
    if (!staffId) { res.status(400).end(); return; }
    ...
    const result = await pool.query(
      `SELECT ${pair[0]} AS blob,${pair[1]} AS mime FROM staff_verifications WHERE staff_id=$1 LIMIT 1`,
      [staffId],
    );
    if (!result.rows[0]?.blob) { res.status(404).end(); return; }
    res.setHeader("Content-Type", result.rows[0].mime || "image/jpeg");
    res.setHeader("Cache-Control", "no-store, private");
...
adminRouter.post("/staff/:staffId/verification-decision", async (req, res) => {
  try {
    await ensureTable();
    const staffId = parseIdParam(req, res, "staffId");
    const decision = String(req.body?.decision || "");
    if (!staffId || !["approved", "rejected"].includes(decision)) {
    ...
    const result = await pool.query(
      `UPDATE staff_verifications
       ...
       WHERE staff_id=$1 AND status='submitted'
       RETURNING status,reviewed_at,owner_note`,
      [staffId, decision, safeText(req.body?.note, 1000) || null],
    );
...
export { publicRouter as publicStaffVerificationsRouter };
