...
export function useA4Print(delayMs = 180) {
  ...
  const print = () => {
    ...
    window.setTimeout(async () => {
      ...
      await Promise.all(Array.from(document.querySelectorAll<HTMLImageElement>('.pb-a4-print-root img')).map(image => image.decode().catch(() => {})));
      window.print();
    }, delayMs);
  };
  return { active, print };
...
const PRINT_CSS = `
  ...
  @media print {
    ...
    .pb-a4-print-root :is(.overflow-x-auto,.overflow-auto,.overflow-hidden,.max-h-screen) { overflow: visible !important; max-height: none !important; }
    .pb-a4-print-root :is(.grid,.flex) { min-width: 0 !important; }
    .pb-a4-print-root table { width: 100% !important; max-width: 100% !important; table-layout: fixed !important; border-collapse: collapse !important; }
    .pb-a4-print-root thead { display: table-header-group !important; }
    .pb-a4-print-root tfoot { display: table-row-group !important; }
    .pb-a4-print-root th, .pb-a4-print-root td { overflow-wrap: anywhere !important; word-break: break-word !important; white-space: normal !important; height: auto !important; }
    .pb-a4-print-root :is(p,div,span) { overflow-wrap: anywhere; }
