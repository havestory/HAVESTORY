  ...
  return (
    <div className="label-print-target" style={{ width, height, minHeight: height, boxSizing: 'border-box', display: 'flex', flexDirection: 'column', overflow: 'hidden', border: '1px solid #d9d0dc', background: '#fff', color: '#151019', fontFamily: 'Arial,sans-serif', fontSize: 12, boxShadow: '0 26px 70px rgba(35,20,43,.2)' }}>
      ...
      <main style={{ padding: isA5 ? '18px 20px 14px' : '13px 15px 10px', flex: '1 1 auto', minHeight: 0 }}>
        ...
        {schedule && <div style={{ marginTop: 9, padding: isA5 ? '8px 12px' : '6px 8px', border: '2px solid #171217', background: '#fff', color: '#111', textAlign: 'center', lineHeight: 1.35 }}><div style={{ fontFamily: 'Arial, sans-serif', fontSize: isA5 ? 11 : 8, fontWeight: 700 }}>මෙම පාර්සලය {schedule.sinhalaDate && <b>{schedule.sinhalaDate}</b>}{schedule.time24 && <> දින <b>{schedule.time24}</b> ට පෙර බාර දෙන්න.</>}</div><div style={{ fontSize: isA5 ? 10 : 7, fontWeight: 700 }}>Please deliver this parcel before {schedule.englishDate}{schedule.englishTime ? ` at ${schedule.englishTime}.` : '.'}</div></div>}
