...
const STATUS_COLORS = ["hsl(var(--admin-brand-ink))", "hsl(var(--admin-warning))", "hsl(var(--admin-chart-info))", "hsl(var(--admin-success))", "hsl(var(--admin-danger))", "hsl(var(--admin-muted))"];
...
function Change({ value, inverse = false }: { value: number; inverse?: boolean }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-xs font-bold ${positive ? "text-admin-success" : "text-admin-danger"}`}>
...
function MetricCard({ label, value, change, icon: Icon, tone = "bronze", inverse = false, detail }: {
  ...
  const tones = {
    bronze: "bg-admin-warning-soft text-admin-warning border-admin-warning-line",
    green: "bg-admin-success-soft text-admin-success border-admin-success-line",
    blue: "bg-admin-brand-soft text-admin-brand-ink border-admin-brand-line",
...
const statusClass = (status: string) => {
  ...
  if (key === "completed" || key === "paid") return "bg-admin-success-soft text-admin-success";
  if (key === "cancelled" || key === "overdue") return "bg-admin-danger-soft text-admin-danger";
  if (key === "processing" || key === "in_progress") return "bg-admin-brand-soft text-admin-brand-ink";
    ...
    <div className="space-y-6 pb-8">
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1.7fr)_minmax(320px,0.8fr)]">
        <div className="rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-6">
          <div className="flex items-start justify-between gap-3">
            <div><h2 className="font-sans text-xl font-bold text-foreground">Earnings & expenses</h2><p className="text-xs text-muted-foreground">Financial movement during the selected period</p></div>
            <Link href="/admin/finance" className="inline-flex items-center gap-1 text-xs font-bold text-admin-brand-ink">Finance <ChevronRight size={14} /></Link>
          </div>
          <div className="mt-6 h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trend} margin={{ left: 0, right: 6, top: 6, bottom: 0 }}>
                <defs>
                  <linearGradient id="revenueFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="hsl(var(--admin-brand-ink))" stopOpacity={0.32}/><stop offset="100%" stopColor="hsl(var(--admin-brand-ink))" stopOpacity={0.02}/></linearGradient>
                  <linearGradient id="expenseFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="hsl(var(--admin-danger))" stopOpacity={0.18}/><stop offset="100%" stopColor="hsl(var(--admin-danger))" stopOpacity={0.01}/></linearGradient>
                </defs>
                ...
                <Tooltip formatter={(value: number) => money(value)} contentStyle={{ borderRadius: 12, borderColor: "hsl(var(--border))", background: "hsl(var(--card))", color: "hsl(var(--foreground))", fontSize: 12 }} />
                <Legend iconType="circle" wrapperStyle={{ fontSize: 11, paddingTop: 12 }} />
                <Area type="monotone" dataKey="revenue" name="Revenue" stroke="hsl(var(--admin-brand-ink))" strokeWidth={2.5} fill="url(#revenueFill)" />
                <Area type="monotone" dataKey="expenses" name="Expenses" stroke="hsl(var(--admin-danger))" strokeWidth={2} fill="url(#expenseFill)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-6">
          <div><h2 className="font-sans text-xl font-bold text-foreground">Order pipeline</h2><p className="text-xs text-muted-foreground">All active and completed records</p></div>
          {totalStatuses > 0 ? <>
            <div className="relative mt-4 h-[190px]">
              <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={data.orderStatuses} dataKey="count" nameKey="status" innerRadius={55} outerRadius={78} paddingAngle={3}>{data.orderStatuses.map((_, index) => <Cell key={index} fill={STATUS_COLORS[index % STATUS_COLORS.length]} />)}</Pie><Tooltip contentStyle={{ borderRadius: 12, borderColor: "hsl(var(--border))", background: "hsl(var(--card))", color: "hsl(var(--foreground))", fontSize: 12 }} /></PieChart></ResponsiveContainer>
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.75fr)]">
        <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
          <div className="flex items-center justify-between border-b border-border px-5 py-4"><div><h2 className="font-sans text-lg font-bold">Recent orders</h2><p className="text-xs text-muted-foreground">Latest customer work entering the studio</p></div><Link href="/admin/orders" className="text-xs font-bold text-admin-brand-ink">View all</Link></div>
          {data.recentOrders.length ? <div className="divide-y divide-border">{data.recentOrders.map(order => <div key={order.id} className="grid grid-cols-[1fr_auto] items-center gap-3 px-5 py-3.5 hover:bg-muted/40"><div className="min-w-0"><div className="flex items-center gap-2"><span className="font-mono text-[11px] font-bold text-admin-brand-ink">{order.orderId}</span><span className={`rounded-full px-2 py-0.5 text-[9px] font-bold uppercase ${statusClass(order.status)}`}>{order.status.replace(/_/g," ")}</span></div><div className="mt-1 truncate text-sm font-bold text-foreground">{order.customerName}</div></div><div className="text-right text-[11px] text-muted-foreground"><div>{new Date(order.createdAt).toLocaleDateString("en-LK")}</div>{order.dueDate && <div className="mt-0.5 inline-flex items-center gap-1"><Clock3 size={11}/> Due {new Date(order.dueDate).toLocaleDateString("en-LK")}</div>}</div></div>)}</div> : <div className="p-10 text-center text-sm text-muted-foreground">No orders have been created yet.</div>}
        </div>

        <div className="space-y-5">
          <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
            <h2 className="font-sans text-lg font-bold">Quick actions</h2>
            <div className="mt-4 grid grid-cols-2 gap-2">
              {[{href:"/admin/orders",label:"New order",icon:Plus},{href:"/admin/invoices",label:"Invoice",icon:FileText},{href:"/admin/clients",label:"Client",icon:Users},{href:"/admin/finance",label:"Add expense",icon:Banknote}].map(action => <Link key={action.label} href={action.href} className="flex items-center gap-2 rounded-xl border border-border bg-background px-3 py-3 text-xs font-bold text-foreground hover:border-secondary hover:text-admin-brand-ink"><action.icon size={15}/>{action.label}</Link>)}
            </div>
          </div>
          <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
            <div className="flex items-center justify-between"><h2 className="font-sans text-lg font-bold">Needs attention</h2><TriangleAlert size={17} className="text-admin-warning" /></div>
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        <div className="rounded-2xl border border-border bg-card p-5 shadow-sm"><div className="flex items-center justify-between"><div><h2 className="font-sans text-lg font-bold">Orders created</h2><p className="text-xs text-muted-foreground">Volume trend for capacity planning</p></div><CalendarDays size={18} className="text-admin-brand-ink"/></div><div className="mt-4 h-[210px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={trend}><CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" vertical={false}/><XAxis dataKey="label" tick={{fontSize:10,fill:"hsl(var(--muted-foreground))"}} axisLine={false} tickLine={false} minTickGap={22}/><YAxis allowDecimals={false} tick={{fontSize:10,fill:"hsl(var(--muted-foreground))"}} axisLine={false} tickLine={false} width={28}/><Tooltip contentStyle={{ borderRadius: 12, borderColor: "hsl(var(--border))", background: "hsl(var(--card))", color: "hsl(var(--foreground))", fontSize: 12 }}/><Bar dataKey="orders" name="Orders" fill="hsl(var(--admin-brand-ink))" radius={[5,5,0,0]}/></BarChart></ResponsiveContainer></div></div>
        <div className="rounded-2xl border border-border bg-card p-5 shadow-sm"><div className="flex items-center justify-between"><div><h2 className="font-sans text-lg font-bold">Revenue mix</h2><p className="text-xs text-muted-foreground">Income categories in {period}</p></div><CircleDollarSign size={18} className="text-admin-brand-ink"/></div>{data.revenueCategories.length ? <div className="mt-4 space-y-3">{data.revenueCategories.map((item,index)=>{const max=data.revenueCategories[0]?.total||1;return <div key={item.category}><div className="mb-1 flex justify-between gap-3 text-xs"><span className="font-semibold capitalize">{item.category.replace(/_/g," ")}</span><b>{money(item.total)}</b></div><div className="h-2 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full" style={{width:`${Math.max(4,item.total/max*100)}%`,background:STATUS_COLORS[index%STATUS_COLORS.length]}}/></div></div>})}</div>:<div className="flex h-[210px] items-center justify-center text-sm text-muted-foreground">Revenue appears here when invoices are paid.</div>}</div>
      </section>
