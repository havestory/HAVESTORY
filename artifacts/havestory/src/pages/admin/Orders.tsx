  ...
  return (
    <div className="space-y-6 pb-10">
            ...
            <div className="space-y-5">
              <div className="relative space-y-2">
                ...
                <div className="relative">
                  <Search className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-admin-muted" />
                  <Input value={selectedClient ? selectedClient.name : clientSearch} onFocus={() => setCustomerMenuOpen(true)} onChange={(event) => { setCreateForm((form) => ({ ...form, clientId: null, isNewClient: true, customerName: event.target.value })); setClientSearch(event.target.value); setCustomerMenuOpen(true); }} placeholder="Search by name, phone, email, business, or PB-code..." className="h-11 rounded-full border-admin-brand-line bg-admin-surface pl-10 text-sm shadow-[0_4px_12px_rgba(220,40,150,0.08)] focus-visible:ring-admin-brand" />
                ...
                {customerMenuOpen && !selectedClient && <div className="absolute left-0 right-0 top-[68px] z-20 overflow-hidden rounded-2xl border border-admin-border bg-admin-surface shadow-xl">
                  ...
                  <button type="button" onClick={() => { setCreateForm((form) => ({ ...form, clientId: null, isNewClient: true, customerName: clientSearch || form.customerName })); setCustomerMenuOpen(false); }} className="flex w-full items-center gap-2 border-t border-admin-brand-line bg-admin-brand px-4 py-3 text-left text-sm font-bold text-admin-brand-ink hover:"><UserRoundPlus className="h-4 w-4" /> + Add a new client</button>
                </div>}
              </div>
      ...
      <Dialog open={manageOpen} onOpenChange={setManageOpen}>
        <DialogContent className="max-w-[700px] overflow-hidden rounded-[28px] border-0 bg-admin-surface p-0 shadow-2xl">
