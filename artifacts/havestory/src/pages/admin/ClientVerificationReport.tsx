...
export default function ClientVerificationReport() {
  ...
  const report = <article id="client-verification-report" className="verification-a4 pb-print-flow mx-auto max-w-[900px] rounded-3xl bg-white p-6 shadow-xl sm:p-9">
      <header className="flex flex-wrap items-start justify-between gap-4 border-b-2 border-slate-900 pb-5">
        <div><div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-amber-600"><ShieldCheck size={18} /> {businessName} · Owner Confidential</div><h1 className="mt-2 text-3xl font-black text-slate-950">Client Verification Report</h1><p className="mt-1 text-sm text-slate-500">Profile {String(client.id).padStart(4, "0")} · Generated {new Date().toLocaleString("en-LK")}</p></div>
