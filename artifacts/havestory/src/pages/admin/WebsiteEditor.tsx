        ...
        <div className="border-2 border-dashed border-blue-200 rounded-2xl p-4 bg-blue-50/40 space-y-3">
          ...
          <div className="flex items-start gap-2 text-[10px] text-blue-600">
            ...
            <span>
              ...
              image address" → paste below. Or use any direct image URL from the
              web.
            </span>
          </div>
...
export default function WebsiteEditor() {
  const [tab, setTab] = useState<TabId>(() =>
    window.location.pathname.endsWith("/admin/portfolio")
      ? "portfolio"
      : "home",
  );
  ...
  const [heroSlideEnabled, setHeroSlideEnabled] = useState<boolean[]>(() =>
    Array(10).fill(true),
  );
  ...
  const { mutate: createPortfolio, isPending: creatingP } =
    useCreatePortfolioItem({
      mutation: {
        onSuccess: () => {
  ...
  const { mutate: updatePortfolio, isPending: updatingP } =
    useUpdatePortfolioItem({
      mutation: {
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: ["/api/portfolio"] });
  ...
  async function handleGalleryFiles(files: FileList) {
    ...
    for (const file of Array.from(files)) {
      try {
        urls.push(await uploadPortfolioImage(file));
      } catch {
        /* skip */
      }
    }
  ...
  const setF = (k: keyof ReturnType<typeof emptyPForm>, v: any) =>
    setPForm((f) => ({ ...f, [k]: v }));
  ...
  useEffect(() => {
    ...
    try {
      const parsed =
        typeof s.heroSlideEnabled === "string"
          ? JSON.parse(s.heroSlideEnabled)
          : s.heroSlideEnabled;
      setHeroSlideEnabled(
        Array.from({ length: 10 }, (_, index) =>
          Array.isArray(parsed) ? parsed[index] !== false : true,
        ),
      );
    } catch {
      setHeroSlideEnabled(Array(10).fill(true));
    }
    setDesignerCredit(s.designerCredit ?? "");
    try {
      const parsed =
        typeof s.homeFeatureCards === "string"
          ? JSON.parse(s.homeFeatureCards)
          : s.homeFeatureCards;
      setHomeFeatureCards(
        Array.isArray(parsed) && parsed.length
          ? DEFAULT_HOME_FEATURE_CARDS.map((fallback, index) => ({
              ...fallback,
              ...(parsed[index] || {}),
            }))
          : DEFAULT_HOME_FEATURE_CARDS,
        ...
        {tab === "home" && (
          <div>
            <SectionHeader
              icon={Home}
              title="Home Page"
              desc="Edit your homepage hero section — the first thing visitors see"
            />
            ...
            <div className="space-y-6">
                    ...
                    >
                      <div className="mb-3 flex items-center justify-between gap-3 rounded-xl border border-gray-100 bg-white px-3 py-2">
                        ...
                        <button
                          type="button"
                          disabled={!value}
                          aria-pressed={heroSlideEnabled[index]}
                          onClick={() =>
                            setHeroSlideEnabled((items) =>
                              items.map((enabled, itemIndex) =>
                        ...
                        >
                          ...
                          {heroSlideEnabled[index] ? "Published" : "Hidden"}
                        </button>
                      </div>
                      <SiteImageUpload
                        label={label as string}
                        value={value as string}
                        onChange={onChange as (url: string) => void}
                        sizeHint="800 × 1000 px portrait"
              ...
              <div>
                ...
                <div className="flex gap-4">
                  <AvatarSlot
                    label="Photo 1"
                    url={heroAvatarImage1}
                    onChange={setHeroAvatarImage1}
                  />
                  <AvatarSlot
                    label="Photo 2"
                    url={heroAvatarImage2}
                    onChange={setHeroAvatarImage2}
                  />
                  <AvatarSlot
                    label="Photo 3"
                    url={heroAvatarImage3}
                    onChange={setHeroAvatarImage3}
                  />
                  <AvatarSlot
                    label="Photo 4"
                    url={heroAvatarImage4}
                    onChange={setHeroAvatarImage4}
                  />
                </div>
              </div>
        ...
        {tab === "about" && (
          <div>
            <SectionHeader
              icon={Users}
              title="About Us"
              desc="Edit the About page — your story, mission, and company details"
            />
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input
                    value={aboutTeamSize}
                    onChange={(e) => setAboutTeamSize(e.target.value)}
                    placeholder="10+"
                    className={inp}
                  />
                </div>
          ...
          <div>
            <div className="flex items-start justify-between mb-6">
              <SectionHeader
                icon={Images}
                title="Portfolio"
                desc="Manage your portfolio projects — add multiple images per project for the gallery slider"
              />
              {!showPForm && (
                <button
                  onClick={() => {
              ...
              <div className="mb-6 border border-amber-100 rounded-2xl bg-amber-50/30 p-5 space-y-5">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      const f = e.target.files?.[0];
                      if (f) {
                        const url = await uploadPortfolioImage(f);
                        setPCoverUrl(url);
                      }
                      e.target.value = "";
                    }}
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      value={pForm.completedAt}
                      onChange={(e) => setF("completedAt", e.target.value)}
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="wedding, luxury, gold foil"
                    className={inp}
                  />
                </div>
        ...
        {tab === "privacy" && (
          <div>
            <SectionHeader
              ...
              desc="Displayed on /privacy — edit your full privacy policy text"
            />
        ...
        {tab === "terms" && (
          <div>
            <SectionHeader
              ...
              desc="Displayed on /terms — edit your full terms and conditions"
            />
...
For questions about these terms, email us at ${(settings as any)?.email || "the email configured in General Settings"}`}
        ...
        {tab === "seo" && (
          <div>
            <SectionHeader
              icon={Search}
              title="SEO Settings"
              desc="Control how your website appears in Google search results and social shares"
            />
        ...
        {tab === "icon" && (
          <div>
            <SectionHeader
              ...
              desc="The small icon shown in the browser tab, bookmarks, and shortcuts"
            />
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-start gap-3 px-4 py-3 bg-blue-50 border border-blue-100 rounded-xl">
                ...
                <div className="text-xs text-blue-700">
                  ...
                  use a <strong>.ico file</strong> at 32×32 px or 64×64 px.
                </div>
              </div>
            </div>
        ...
        {tab === "theme" && (
          <div>
            <SectionHeader
              icon={Palette}
              title="Color Theme"
              desc="Choose a color scheme for buttons, accents, and gradients across the site"
            />
                ...
                return (
                  <button
                    ...
                    style={
                      active
                        ? {
                            borderColor: preset.from,
                            background: `linear-gradient(135deg, ${preset.from}15, ${preset.to}15)`,
                          }
                        : {}
                    }
                  >
              ...
              >
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setSpecialEventMessage(event.target.value)
                    }
                    disabled={!specialEventEnabled}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (deleteConfirm) deletePortfolio({ id: deleteConfirm.id });
        }}
