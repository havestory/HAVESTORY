...
const statusStyle: Record<string, string> = {
  ...
  in_progress: "bg-admin-brand-soft text-admin-brand-ink",
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize ${statusStyle[normalizedStatus] || "bg-admin-subtle text-admin-muted"}`}>
...
export default function CRMProjects() {
  ...
  const inp = "w-full px-3.5 py-2.5 border border-admin-border rounded-xl text-sm outline-none focus:border-admin-brand-line transition-colors placeholder:text-admin-muted";
    ...
    <div className="space-y-5">
      <div className="flex items-start sm:items-center justify-between gap-3 flex-wrap">
        <div>
          <div className="flex items-center gap-2">
            <FolderKanban size={22} className="text-admin-brand-ink" />
            <h1 className="text-xl sm:text-2xl font-bold text-admin-ink">CRM Projects</h1>
          </div>
          <p className="text-xs sm:text-sm text-admin-muted">Track your printing projects from start to finish</p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <button onClick={fetchProjects} className="hidden sm:flex items-center gap-1.5 px-3 py-2 rounded-lg border border-admin-border text-sm text-admin-muted hover:bg-admin-surface">
            <RefreshCw size={14} /> Refresh
          </button>
          <button
            onClick={openTypeModal}
            className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-admin-brand-line bg-admin-brand-soft text-admin-brand-ink text-sm font-semibold hover:bg-admin-brand-soft transition-colors"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
        {[
          ...
          <div key={c.label} className={`bg-admin-surface border border-admin-border rounded-xl px-4 py-3.5 shadow-sm${i === arr.length - 1 ? " col-span-2 sm:col-span-1" : ""}`}>
      ...
      <div className="space-y-2 sm:space-y-0 sm:flex sm:items-center sm:gap-3 sm:flex-wrap">
        <div className="relative flex-1 min-w-0 sm:min-w-48">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search projects..." className="w-full pl-9 pr-4 py-2 border border-admin-border rounded-xl text-sm outline-none focus:ring-2 focus:ring-admin-brand" />
        </div>
        <div className="flex items-center gap-1.5 flex-wrap">
          <div className="flex gap-1 flex-wrap flex-1">
            <button onClick={() => setStatusFilter("all")} className={`px-2.5 sm:px-3 py-1.5 sm:py-2 text-[11px] sm:text-xs font-semibold rounded-lg border transition-all ${statusFilter === "all" ? "bg-primary text-primary-foreground border-primary" : "border-admin-border text-admin-muted hover:border-primary/40"}`}>All</button>
            {STATUS_OPTIONS.map(s => (
              <button key={s} onClick={() => setStatusFilter(s)} className={`px-2.5 sm:px-3 py-1.5 sm:py-2 text-[11px] sm:text-xs font-semibold rounded-lg border capitalize transition-all ${statusFilter === s ? "bg-primary text-primary-foreground border-primary" : "border-admin-border text-admin-muted hover:border-primary/40"}`}>
                {s.replace("_", " ")}
              </button>
            ))}
          </div>
      ...
      <div className="bg-admin-surface border border-admin-border rounded-2xl shadow-sm overflow-hidden">
        {loading ? (
          <div className="space-y-2 p-4">
            {[1, 2, 3].map(i => <div key={i} className="h-16 bg-admin-subtle rounded-xl animate-pulse" />)}
          </div>
        ) : loadError ? (
          <div className="py-20 px-6 text-center">
            <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-admin-danger-soft text-admin-danger">!</div>
            <p className="text-base font-semibold text-admin-ink">CRM projects could not be loaded</p>
            <p className="mx-auto mt-1 max-w-md text-sm text-admin-muted">{loadError}</p>
            <button type="button" onClick={() => void fetchProjects()} className="mt-5 inline-flex items-center gap-2 rounded-lg bg-admin-inverse px-4 py-2 text-sm font-semibold text-white hover:bg-admin-inverse">
          ...
          <div className="py-20 text-center text-admin-muted">
            ...
            <p className="text-base font-medium text-admin-muted">{search || statusFilter !== "all" ? "No projects match your filter" : "No projects yet"}</p>
            {!search && statusFilter === "all" && <p className="text-sm text-admin-muted mt-1">Click "New Project" to create your first project</p>}
          </div>
        ) : (
          <>
            ...
            <div className="sm:hidden divide-y divide-admin-border">
              {filtered.map(p => {
                ...
                return (
                  <div key={p.id}>
                    <div
                      className="px-4 py-3.5 cursor-pointer active:bg-admin-surface"
                      onClick={() => setExpanded(isExpanded ? null : p.id)}
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <span className="font-semibold text-admin-ink text-sm leading-tight">{p.title}</span>
                            {isExpanded ? <ChevronUp size={12} className="text-admin-muted shrink-0" /> : <ChevronDown size={12} className="text-admin-muted shrink-0" />}
                          </div>
                          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                            <span className="text-[10px] text-admin-brand-ink font-mono">{p.projectId}</span>
                            <span className="text-xs text-admin-muted">{p.clientName}</span>
                            {p.serviceTypeId && typeMap[p.serviceTypeId] && (
                              <span className="text-[10px] font-bold px-1.5 py-0.5 bg-admin-brand-soft text-admin-brand-ink rounded-full border border-admin-brand-line">{typeMap[p.serviceTypeId]}</span>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="mt-2 flex items-center gap-3 text-xs flex-wrap">
                        <span className="text-admin-muted">Val: <span className="font-semibold text-admin-ink">{rs(p.totalValue)}</span></span>
                        <span className="text-admin-muted">Paid: <span className="font-semibold text-admin-success">{rs(p.amountPaid)}</span></span>
                        {balance > 0 && <span className="text-admin-muted">Bal: <span className="font-semibold text-admin-danger">{rs(balance)}</span></span>}
                        {p.dueDate && <span className="text-admin-muted ml-auto">Due: {p.dueDate}</span>}
                      </div>
                    </div>
                    {isExpanded && (
                      <div className="bg-admin-surface px-4 py-3 space-y-2 border-t border-admin-border">
                        {p.description && (
                          <div>
                            <span className="text-[10px] font-bold text-admin-muted uppercase block mb-0.5">Description</span>
                            <p className="text-admin-ink text-xs">{p.description}</p>
                          </div>
                        )}
                        {p.notes && (
                          <div>
                            <span className="text-[10px] font-bold text-admin-muted uppercase block mb-0.5">Notes</span>
                            <p className="text-admin-muted text-xs bg-admin-surface rounded-lg px-2.5 py-1.5 border border-admin-border">{p.notes}</p>
                          </div>
                        )}
                        <div className="flex items-center gap-3 pt-0.5">
                          {p.startDate && <span className="text-[10px] text-admin-muted">Start: {p.startDate}</span>}
                          {p.dueDate && <span className="text-[10px] text-admin-muted">Due: {p.dueDate}</span>}
                          <div className="text-[10px] text-admin-muted">
                            Bal: <span className={`font-semibold ${balance > 0 ? "text-admin-danger" : "text-admin-success"}`}>{rs(balance)}</span>
                          </div>
                          <div className="ml-auto flex items-center gap-1" onClick={e => e.stopPropagation()}>
                            <button onClick={() => openEdit(p)} className="inline-flex items-center justify-center p-2 hover:bg-admin-brand-soft text-admin-muted hover:text-admin-brand-ink rounded-lg"><Edit2 size={14} /></button>
                            <button onClick={() => setDeleteId(p.id)} className="inline-flex items-center justify-center p-2 hover:bg-admin-danger-soft text-admin-muted hover:text-admin-danger rounded-lg"><Trash2 size={14} /></button>
                          </div>
                        </div>
                      </div>
                    )}
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-admin-border">
                {filtered.map(p => {
                  ...
                  return (
                    <Fragment key={p.id}>
                      <tr key={p.id} className="hover:bg-admin-surface/50 transition-colors cursor-pointer" onClick={() => setExpanded(isExpanded ? null : p.id)}>
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <span className="font-semibold text-admin-ink">{p.title}</span>
                            {isExpanded ? <ChevronUp size={14} className="text-admin-muted" /> : <ChevronDown size={14} className="text-admin-muted" />}
                            {p.serviceTypeId && typeMap[p.serviceTypeId] && (
                              <span className="text-[10px] font-bold px-1.5 py-0.5 bg-admin-brand-soft text-admin-brand-ink rounded-full border border-admin-brand-line">{typeMap[p.serviceTypeId]}</span>
                            )}
                          </div>
                          <div className="text-[10px] text-admin-brand-ink font-mono mt-0.5">{p.projectId}</div>
                        </td>
                        <td className="px-5 py-3.5 text-admin-ink">{p.clientName}</td>
                        <td className="px-5 py-3.5"><StatusBadge status={p.status} /></td>
                        <td className="px-5 py-3.5 font-semibold text-admin-ink">{rs(p.totalValue)}</td>
                        <td className="px-5 py-3.5">
                          <div className="font-semibold text-admin-success">{rs(p.amountPaid)}</div>
                          {balance > 0 && <div className="text-[10px] text-admin-danger">Bal: {rs(balance)}</div>}
                        </td>
                        <td className="px-5 py-3.5 text-admin-muted text-xs">{p.dueDate || "—"}</td>
                        <td className="px-5 py-3.5 text-right" onClick={e => e.stopPropagation()}>
                          <div className="flex items-center gap-1.5 justify-end">
                            <button onClick={() => openEdit(p)} className="inline-flex items-center justify-center p-1.5 hover:bg-admin-brand-soft text-admin-muted hover:text-admin-brand-ink rounded-lg transition-colors"><Edit2 size={14} /></button>
                            <button onClick={() => setDeleteId(p.id)} className="inline-flex items-center justify-center p-1.5 hover:bg-admin-danger-soft text-admin-muted hover:text-admin-danger rounded-lg transition-colors"><Trash2 size={14} /></button>
                          </div>
                        </td>
                      </tr>
                      {isExpanded && (
                        <tr key={`${p.id}-exp`} className="bg-admin-surface/50">
                          <td colSpan={7} className="px-8 py-4">
                            <div className="grid grid-cols-3 gap-4 text-sm">
                              {p.description && (
                                <div className="col-span-3">
                                  <span className="text-xs text-admin-muted font-semibold block mb-1">DESCRIPTION</span>
                                  <p className="text-admin-ink">{p.description}</p>
                                </div>
                              )}
                              {p.startDate && <div><span className="text-xs text-admin-muted font-semibold block mb-0.5">START DATE</span><span className="text-admin-ink">{p.startDate}</span></div>}
                              {p.dueDate && <div><span className="text-xs text-admin-muted font-semibold block mb-0.5">DUE DATE</span><span className="text-admin-ink">{p.dueDate}</span></div>}
                              <div><span className="text-xs text-admin-muted font-semibold block mb-0.5">BALANCE</span><span className={`font-semibold ${balance > 0 ? "text-admin-danger" : "text-admin-success"}`}>{rs(balance)}</span></div>
                              {p.notes && (
                                <div className="col-span-3">
                                  <span className="text-xs text-admin-muted font-semibold block mb-1">NOTES</span>
                                  <p className="text-admin-muted bg-admin-surface rounded-xl px-3 py-2 border border-admin-border">{p.notes}</p>
                                </div>
                              )}
                            </div>
                          </td>
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            <div className="bg-admin-surface rounded-2xl shadow-2xl w-full max-w-lg" style={{ maxHeight: "calc(100vh - 48px)" }}>
              <div className="flex items-center justify-between p-6 border-b border-admin-border">
                <div>
                  <h2 className="font-bold text-lg text-admin-ink">{editing ? "Edit Project" : "New Project"}</h2>
                  <p className="text-xs text-admin-muted mt-0.5">{editing ? `Editing ${editing.projectId}` : "Create a new CRM project"}</p>
                </div>
                <button onClick={() => setShowModal(false)} className="inline-flex items-center justify-center p-1.5 hover:bg-admin-subtle rounded-lg"><X size={18} /></button>
              </div>
              <form onSubmit={handleSave} className="p-6 space-y-4 overflow-y-auto" style={{ maxHeight: "calc(100vh - 200px)" }}>
                <div>
                  <label className="text-xs text-admin-muted font-semibold block mb-1.5">PROJECT TITLE *</label>
                ...
                <div>
                  <label className="text-xs text-admin-muted font-semibold block mb-1.5">STATUS</label>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="text-xs text-admin-muted font-semibold">SERVICE TYPE</label>
                    <button type="button" onClick={openTypeModal} className="text-[11px] text-admin-brand-ink hover:text-admin-brand-ink font-semibold">+ Manage</button>
                  </div>
                ...
                <div>
                  <label className="text-xs text-admin-muted font-semibold block mb-1.5">DESCRIPTION</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs text-admin-muted font-semibold block mb-1.5">TOTAL VALUE (Rs.)</label>
                  ...
                  <div>
                    <label className="text-xs text-admin-muted font-semibold block mb-1.5">AMOUNT PAID (Rs.)</label>
                  ...
                  <div>
                    <label className="text-xs text-admin-muted font-semibold block mb-1.5">START DATE</label>
                    <input type="date" value={form.startDate} onChange={e => setForm(f => ({ ...f, startDate: e.target.value }))} className={inp} />
                  </div>
                  <div>
                    <label className="text-xs text-admin-muted font-semibold block mb-1.5">DUE DATE</label>
                    <input type="date" value={form.dueDate} onChange={e => setForm(f => ({ ...f, dueDate: e.target.value }))} className={inp} />
                  </div>
                </div>
                <div>
                  <label className="text-xs text-admin-muted font-semibold block mb-1.5">NOTES</label>
                ...
                {saveError && <div className="text-admin-danger text-sm bg-admin-danger-soft border border-admin-danger-line px-3 py-2 rounded-xl">{saveError}</div>}

                <div className="flex gap-3 pt-1">
                  <button type="button" onClick={() => setShowModal(false)} className="flex-1 py-2.5 border border-admin-border rounded-xl text-sm font-semibold text-admin-muted hover:bg-admin-surface">Cancel</button>
      ...
      {showTypeModal && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-admin-inverse/60 backdrop-blur-sm">
          <div className="flex min-h-full items-start sm:items-center justify-center p-3 sm:p-4">
            <div className="bg-admin-surface rounded-2xl shadow-2xl w-full max-w-xl my-4 sm:my-0" style={{ maxHeight: "calc(100vh - 32px)" }}>
              <div className="flex items-center justify-between p-4 sm:p-5 border-b border-admin-border sticky top-0 bg-admin-surface rounded-t-2xl">
                <div>
                  <h2 className="font-bold text-base sm:text-lg text-admin-ink flex items-center gap-2"><Tag size={18} className="text-admin-brand-ink" /> Service Types</h2>
                  <p className="text-[11px] sm:text-xs text-admin-muted mt-0.5">Add, edit or delete project service types</p>
                </div>
                <button onClick={() => { setShowTypeModal(false); cancelTypeEdit(); }} className="inline-flex items-center justify-center p-1.5 hover:bg-admin-subtle rounded-lg"><X size={18} /></button>
              </div>

              <div className="p-4 sm:p-5 space-y-4 overflow-y-auto" style={{ maxHeight: "calc(100vh - 130px)" }}>
                {/* Add / Edit form */}
                <form onSubmit={saveType} className="bg-admin-surface border border-admin-border rounded-xl p-3 sm:p-4">
                  <div className="text-xs font-bold text-admin-muted uppercase tracking-wide mb-2">{typeEditing ? `Editing: ${typeEditing.name}` : "New Service Type"}</div>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3">
                    <div className="sm:col-span-2">
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 border border-admin-border rounded-lg text-sm outline-none focus:ring-2 focus:ring-admin-brand"
                        required
                      />
                    </div>
                    <div>
                  ...
                  {typeFormError && <p className="text-xs text-admin-danger mt-2">{typeFormError}</p>}
                  <div className="flex gap-2 mt-3">
                    ...
                    {typeEditing && (
                      <button type="button" onClick={cancelTypeEdit} className="px-3 py-2 text-xs sm:text-sm text-admin-muted border border-admin-border rounded-lg hover:bg-admin-subtle">Cancel</button>
                    )}
                  </div>
                </form>
                ...
                <div className="border border-admin-border rounded-xl overflow-hidden">
                  {serviceTypes.length === 0 ? (
                    <div className="text-center py-10 text-admin-muted">
                      ...
                      <p className="text-xs mt-1 text-admin-muted">Add one above to start categorizing projects</p>
                    </div>
                  ) : (
                    <ul className="divide-y divide-admin-border">
                      {serviceTypes.map(t => {
                        ...
                        return (
                          <li key={t.id} className={`flex items-center justify-between gap-2 px-3 sm:px-4 py-2.5 sm:py-3 ${isEditing ? "bg-admin-brand-soft" : "hover:bg-admin-surface"}`}>
                            <div className="min-w-0 flex-1 flex items-center gap-2">
                              <span className="font-semibold text-admin-ink text-sm truncate">{t.name}</span>
                              <span className="text-[10px] font-bold px-2 py-0.5 bg-admin-brand-soft text-admin-brand-ink rounded-full border border-admin-brand-line shrink-0">{count} project{count === 1 ? "" : "s"}</span>
                            </div>
      ...
      {deleteId !== null && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-admin-inverse/60 backdrop-blur-sm">
          <div className="flex min-h-full items-center justify-center p-4">
            <div className="bg-admin-surface rounded-2xl shadow-xl p-6 w-full max-w-sm text-center">
              <div className="w-12 h-12 bg-admin-danger-soft rounded-full flex items-center justify-center mx-auto mb-4"><Trash2 size={22} className="text-admin-danger" /></div>
              <h3 className="text-lg font-bold text-admin-ink mb-1">Delete Project?</h3>
              <p className="text-sm text-admin-muted mb-6">This project will be permanently deleted and cannot be recovered.</p>
              <div className="flex gap-3">
                <button onClick={() => setDeleteId(null)} className="flex-1 px-4 py-2.5 border border-admin-border rounded-xl text-sm font-semibold text-admin-muted hover:bg-admin-surface">Cancel</button>
                <button onClick={() => handleDelete(deleteId)} className="flex-1 px-4 py-2.5 bg-admin-danger-solid hover:bg-admin-danger-solid text-white rounded-xl text-sm font-bold">Delete</button>
              </div>
            </div>
          </div>
        </div>
