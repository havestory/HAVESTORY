...
const PINK   = "#be185d";
const PURPLE = "#7c3aed";
const GRAD   = "linear-gradient(90deg,#be185d,#7c3aed)";
...
const STATUS_BADGE: Record<string, { bg: string; border: string; color: string; label: string }> = {
  pending:   { bg: "#fdf2f8", border: "#f9a8d4", color: "#be185d", label: "PENDING" },
    ...
    <table style={{ width: "100%", borderCollapse: "collapse" }}>
      ...
      <tbody>
        {chunk.map((it, i) => {
          ...
          const bg = (startIdx + i) % 2 === 0 ? "#fff" : "#fdf2f8";
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-0.5 -mx-1 px-1">
              {onSave && (
                <button onClick={onSave} disabled={isSaving}
                  className="flex items-center gap-1.5 px-3 py-2 border-2 border-pink-200 text-pink-600 text-xs font-bold rounded-xl hover:bg-pink-50 transition-colors disabled:opacity-60 whitespace-nowrap shrink-0">
                  {isSaving ? "Saving…" : "Save Invoice"}
                </button>
              )}
              <button onClick={printInvoice} disabled={generatingPDF || downloadingZip}
                className="flex items-center gap-1.5 px-3 py-2 bg-gradient-to-r from-pink-500 to-purple-600 text-white text-xs font-bold rounded-xl hover:opacity-90 disabled:opacity-60 whitespace-nowrap shrink-0">
              ...
              <div ref={el => { pageRefs.current[itemPageCount] = el; }} style={{ ...pageShell, transform: `scale(${pageScale})`, transformOrigin: "top left" }} className="inv-page">
                <div style={contentArea}>
                  ...
                  {banks.length > 0 ? (
                    <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 22 }}>
                      {banks.map((bank, bi) => (
                        <div key={bi} style={{ borderRadius: 12, overflow: "hidden", border: "1px solid #f3e8ff" }}>
                          ...
                          <div style={{ background: "#fdf2f8", padding: "8px 16px", borderTop: "1px solid #f9a8d4" }}>
                    ...
                    <div style={{ background: "#fdf2f8", borderRadius: 10, padding: "14px 16px", marginBottom: 22, border: "1px solid #f9a8d4" }}>
