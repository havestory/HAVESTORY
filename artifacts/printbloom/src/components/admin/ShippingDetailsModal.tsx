...
export function ShippingDetailsModal({ client, onClose }: { client: { id: number; name: string; phone?: string | null; address?: string | null }; onClose: () => void }) {
  ...
  const input = "w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-amber-400";
  return <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/50 sm:items-center sm:p-4" onClick={onClose}><div className="max-h-[92vh] w-full overflow-y-auto rounded-t-3xl bg-white shadow-2xl sm:max-w-xl sm:rounded-3xl" onClick={event => event.stopPropagation()}>
    <div className="sticky top-0 z-10 flex items-center justify-between border-b border-gray-100 bg-white px-5 py-4"><div className="flex items-center gap-2"><PackageCheck className="text-amber-500" size={19}/><div><h2 className="font-bold text-gray-900">Shipping Details</h2><p className="text-[11px] text-gray-400">{client.name} · saved to customer profile</p></div></div><button onClick={onClose} className="rounded-lg p-2 text-gray-400 hover:bg-gray-100"><X size={18}/></button></div>
    {loading ? <div className="flex justify-center py-20"><Loader2 className="animate-spin text-amber-500"/></div> : <div className="space-y-4 p-5">
      ...
      <button onClick={save} disabled={saving} className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-amber-500 to-stone-600 py-3 text-sm font-bold text-white disabled:opacity-60">{saving?<Loader2 size={16} className="animate-spin"/>:<Save size={16}/>}Save Shipping Details</button>
    </div>}
  </div></div>;
}
