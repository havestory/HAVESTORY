...
export default function StaffVerification() {
  ...
  const [form, setForm] = useState({ fullName: "", nicNumber: "", dateOfBirth: "", phone: "", email: "", address: "", emergencyContactName: "", emergencyContactPhone: "" });
  ...
  useEffect(() => {
    fetch(`/api/staff-verifications/${encodeURIComponent(token || "")}`, { cache: "no-store" })
      .then(async r => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/staff-verifications/${encodeURIComponent(token || "")}/submit`, { method: "POST", body: data });
  ...
  if (status === "submitted" || status === "approved") {
    return <div className="min-h-screen bg-slate-950 px-5 grid place-items-center">
      <div className="w-full max-w-md rounded-3xl bg-white p-8 text-center shadow-2xl">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-500">{status === "approved" ? "Your PrintBloom staff profile is verified." : "Your details are now waiting for PrintBloom owner review."}</p>
      </div>
    </div>;
  }
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_top,#312e81_0,#0f172a_44%,#020617_100%)] px-4 py-8 sm:py-12">
    <div className="mx-auto max-w-2xl overflow-hidden rounded-[2rem] bg-white shadow-2xl">
      <header className="bg-slate-950 px-6 py-7 text-white sm:px-9">
        <div className="flex items-center gap-3 text-xs font-bold uppercase tracking-[0.2em] text-pink-400"><ShieldCheck size={18} /> PrintBloom · Secure staff profile</div><div className="mt-3 inline-flex rounded-lg border border-indigo-400/30 bg-indigo-400/10 px-3 py-1.5 text-[10px] font-black uppercase tracking-wider text-indigo-200">Operated by CodeArtix Technologies</div>
        <h1 className="mt-3 text-3xl font-black">Verify your staff profile</h1>
      ...
      <form onSubmit={submit} className="space-y-7 p-6 sm:p-9">
        <section className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            ...
            <textarea required rows={3} value={form.address} onChange={e => setForm(f => ({ ...f, address: e.target.value }))} className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-pink-500" />
          </div>
        </section>
        ...
        <section>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {captureCards.map(card => {
              ...
              return <button key={card.key} type="button" onClick={() => void openCamera(card.key)} className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 text-left transition hover:border-pink-300">
        ...
        <button disabled={submitting} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-pink-600 to-violet-600 px-5 py-3.5 font-bold text-white shadow-lg disabled:opacity-60">
