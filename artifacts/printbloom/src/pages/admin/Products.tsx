...
function MPLaminationCard({ lam, onChange, onRemove }: { lam: MPLamination; onChange: (l: MPLamination) => void; onRemove: () => void }) {
  return (
    <div className="mb-2 p-3 bg-white rounded-xl border border-purple-200">
      <div className="flex justify-between mb-2">
        <span className="text-xs font-bold text-purple-700">Lamination Option</span>
        <button type="button" onClick={onRemove} className="text-xs text-red-400 hover:text-red-600">Remove</button>
      </div>
      <div className="grid grid-cols-2 gap-2 mb-1">
        <div>
      ...
      {open && (
        <div className="p-4 space-y-4">
          ...
          <div className="p-3 bg-purple-50 rounded-xl border border-purple-200">
            <div className="flex justify-between items-center mb-2">
              <p className="text-xs font-bold text-purple-800">③ Laminations</p>
              <button type="button" onClick={() => onChange({ ...board, laminations: [...board.laminations, emptyMPLamination()] })} className="text-[10px] font-bold px-2 py-1 bg-purple-600 text-white rounded-lg hover:bg-purple-700">+ Add Option</button>
            </div>
            {board.laminations.length === 0 && <p className="text-[11px] text-purple-400 text-center py-2">No lamination options. Click "+ Add Option".</p>}
  ...
  return (
    <div className="space-y-2">
        ...
        <div key={i} className="grid grid-cols-3 gap-2 items-center">
          <div className="flex items-center gap-2">
            <input
              ...
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-pink-200"
              placeholder="100"
            />
          </div>
          <div className="col-span-2 flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-pink-200"
      ...
      <button
        ...
        className="text-xs text-pink-500 font-semibold hover:text-pink-700 flex items-center gap-1 mt-1"
      >
    ...
    <div className="space-y-2">
      ...
      {rows.map((row, i) => (
        <div key={i} className="grid grid-cols-4 gap-2 items-center">
          <input type="number" value={row.from} onChange={e => { const n = [...rows]; n[i] = { ...n[i], from: Number(e.target.value) }; onChange(n); }} className="px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-pink-200" placeholder="1" />
          <input type="number" value={row.to} onChange={e => { const n = [...rows]; n[i] = { ...n[i], to: Number(e.target.value) }; onChange(n); }} className="px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-pink-200" placeholder="5" />
          <input type="number" value={row.pricePerUnit} onChange={e => { const n = [...rows]; n[i] = { ...n[i], pricePerUnit: e.target.value }; onChange(n); }} className="px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-pink-200" placeholder="350" />
      ...
      <button type="button" onClick={() => onChange([...rows, { from: 0, to: 0, pricePerUnit: "" }])} className="text-xs text-pink-500 font-semibold hover:text-pink-700 flex items-center gap-1 mt-1">
  ...
  return (
    <div className="space-y-4">
      {sizes.map((size, sIdx) => (
        <div key={size.id} className="border border-gray-200 rounded-2xl overflow-hidden bg-white">
          <div className="flex items-center gap-2 px-3 sm:px-4 py-3 bg-gray-50 border-b border-gray-100">
            <Ruler size={14} className="text-pink-400 shrink-0" />
            <input
              value={size.name}
          ...
          <div className="p-3 sm:p-4 space-y-3">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-pink-200"
            ...
            <div className="space-y-2">
                ...
                <div key={tIdx} className="relative border border-gray-100 sm:border-0 rounded-xl sm:rounded-none p-3 sm:p-0">
                  <div className="grid grid-cols-3 sm:grid-cols-[1fr_1fr_1fr_auto] gap-2 items-center">
                    <div>
                      ...
                      <input type="number" value={tier.from} onChange={e => updateTier(sIdx, tIdx, { from: Number(e.target.value) })} className="w-full px-2 py-2 sm:py-1.5 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-pink-200" placeholder="20" />
                    </div>
                    <div>
                      <span className="text-[10px] text-gray-400 uppercase font-medium sm:hidden block mb-1">To</span>
                      <input type="number" value={tier.to} onChange={e => updateTier(sIdx, tIdx, { to: Number(e.target.value) })} className="w-full px-2 py-2 sm:py-1.5 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-pink-200" placeholder="80" />
                    </div>
                    <div>
                      <span className="text-[10px] text-gray-400 uppercase font-medium sm:hidden block mb-1">Rs./unit</span>
                      <input type="number" value={tier.pricePerUnit} onChange={e => updateTier(sIdx, tIdx, { pricePerUnit: e.target.value })} className="w-full px-2 py-2 sm:py-1.5 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-pink-200" placeholder="8.00" />
                    </div>
              ...
              <button type="button" onClick={() => addTier(sIdx)} className="text-xs text-pink-500 font-semibold hover:text-pink-700 flex items-center gap-1 py-1">
      ...
      <button
        type="button"
        onClick={addSize}
        className="w-full py-3 border-2 border-dashed border-pink-200 text-pink-500 text-sm font-semibold rounded-2xl hover:bg-pink-50 transition-colors flex items-center justify-center gap-2"
      >
  ...
  return (
    <div className="bg-gray-50 p-2 rounded-xl space-y-2">
      <div className="flex gap-2 items-center">
        <input value={choice.name} onChange={e => onChange({ ...choice, name: e.target.value })} className="flex-1 px-3 py-1.5 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-pink-200" placeholder="e.g. Matte" />
      ...
      {!collapsed && (
        <div className="p-4 space-y-2">
          ...
          <button
            ...
            className="text-xs text-pink-500 font-semibold hover:text-pink-700 flex items-center gap-1 mt-2"
          >
...
function CategoryCombobox({
  ...
  return (
    <div className="relative">
      <div className="flex items-center gap-2 w-full px-4 py-2.5 border border-gray-200 rounded-xl bg-white focus-within:ring-2 focus-within:ring-pink-200">
        <input
          value={query}
        ...
        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden max-h-52 overflow-y-auto">
          ...
          {filtered.map(c => (
            <button
              ...
              className={`w-full text-left px-4 py-2.5 text-sm hover:bg-pink-50 hover:text-pink-700 transition-colors flex items-center gap-2 ${value.id === c.id ? "bg-pink-50 text-pink-700 font-semibold" : "text-gray-700"}`}
            >
              <Tag size={12} className="shrink-0 opacity-50" />
              {c.name}
              {value.id === c.id && <span className="ml-auto text-[10px] text-pink-400 font-bold">Selected</span>}
            </button>
          ))}
          {query.trim() && !exactMatch && (
            <button
              ...
              className="w-full text-left px-4 py-2.5 text-sm text-purple-600 hover:bg-purple-50 transition-colors flex items-center gap-2 border-t border-gray-100 font-semibold"
            >
      ...
      {value.id === null && value.name && (
        <p className="mt-1 text-[11px] text-purple-600 flex items-center gap-1">
      ...
      <div className="flex items-start sm:items-center justify-between gap-3 flex-wrap">
        ...
        {!readOnly&&<div className="flex items-center gap-2 shrink-0 flex-wrap">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-pink-200 bg-pink-50 text-pink-700 text-sm font-semibold hover:bg-pink-100 transition-colors"
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-purple-200 bg-purple-50 text-purple-700 text-sm font-semibold hover:bg-purple-100 transition-colors disabled:opacity-60"
          ...
          <button
            onClick={openAdd}
            className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gradient-to-r from-pink-500 to-purple-600 text-white text-sm font-semibold shadow-md shadow-pink-500/20 hover:opacity-90 transition-opacity"
          >
                ...
                <div className="flex sm:hidden items-center gap-3 p-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-0.5 flex-wrap">
                      {p.category && <span className="text-[9px] font-bold text-pink-500 uppercase tracking-widest">{p.category.name}</span>}
                      {p.featured && <span className="text-[9px] font-bold text-amber-500 uppercase">⭐ Featured</span>}
                      {isCustom && <span className="text-[9px] font-bold text-purple-500 uppercase">Custom</span>}
                    ...
                    <div className="text-xs font-semibold text-pink-600 mt-0.5">{priceLabel}{!isCustom && !isMultiSize && <span className="text-gray-400 font-normal ml-1">/ item</span>}</div>
                ...
                <div className="hidden sm:flex flex-col h-full">
                  ...
                  <div className="relative aspect-[4/3] bg-gray-50 overflow-hidden">
                    ...
                    <div className="absolute top-2 left-2 flex gap-1.5">
                      ...
                      {isCustom && <span className="px-2 py-0.5 bg-purple-500 text-white text-[10px] font-bold rounded-full">Custom Print</span>}
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    {p.category && <span className="text-[10px] font-bold text-pink-500 uppercase tracking-widest mb-1">{p.category.name}</span>}
                    ...
                    {isMultiPrints ? (
                        ...
                        {cfg.pricingModel === "fixed_quantities" && cfg.fixedPrices.filter(x => x.price).length > 0 ? (
                          <div className="flex gap-1 flex-wrap">
                            {cfg.fixedPrices.filter(x => x.price).map((fp, i) => (
                              <span key={i} className="px-2 py-0.5 bg-pink-50 text-pink-700 text-[10px] font-semibold rounded-lg">{fp.qty} pcs · {rs(fp.price)}</span>
                            ))}
                          </div>
                          ...
                          <div className="flex gap-1 flex-wrap">
                            {cfg.rangePrices.filter(x => x.pricePerUnit).map((rp, i) => (
                              <span key={i} className="px-2 py-0.5 bg-purple-50 text-purple-700 text-[10px] font-semibold rounded-lg">{rp.from}–{rp.to} · {rs(rp.pricePerUnit)}/unit</span>
                            ))}
                          </div>
                        ) : (
                      ...
                      <div className="font-bold text-pink-600">{rs(p.price)}<span className="text-xs text-gray-400 font-normal ml-1">/ item</span></div>
                    )}
            ...
            <form onSubmit={handleSubmit} className="flex flex-col flex-1 p-6 space-y-6">
              ...
              <section className="space-y-4">
                ...
                <div>
                  ...
                  <input required value={form.name} onChange={e => f("name", e.target.value)} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-pink-200" placeholder="e.g. Business Cards (Premium)" />
                </div>
                ...
                <div>
                  ...
                  <input value={form.invoiceName || ""} onChange={e => f("invoiceName", e.target.value)} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-pink-200" placeholder="e.g. Biz Cards Premium" />
                </div>
                ...
                <div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      {form.galleryImages.map((url: string, idx: number) => (
                        <div key={url} className="relative group w-20 h-20 rounded-xl overflow-hidden border-2 border-gray-100 shrink-0">
                          ...
                          {/* Cover badge */}
                          {idx === 0 && (
                            <div className="absolute bottom-0 left-0 right-0 bg-pink-500 text-white text-[9px] font-bold text-center py-0.5">Cover</div>
                          )}
                          {/* Actions on hover */}
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2.5 border-2 border-dashed border-gray-200 hover:border-pink-300 rounded-xl text-sm text-gray-400 hover:text-pink-500 transition-all w-full justify-center"
                  >
                    {imgUploading
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-pink-200 bg-white"
                ...
                <div className="flex gap-6">
                  <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
                    <input type="checkbox" checked={form.featured} onChange={e => f("featured", e.target.checked)} className="w-4 h-4 rounded accent-pink-500" />
                    <span>⭐ Featured</span>
                  </label>
                  <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
                    <input type="checkbox" checked={form.active} onChange={e => f("active", e.target.checked)} className="w-4 h-4 rounded accent-pink-500" />
              ...
              <section className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {[
                    ...
                    { value: "multi_prints", label: "Multi Prints", sub: "Board types with print sides & laminations" },
                  ].map(opt => (
                    <button
                      key={opt.value}
                      type="button"
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        <div className={`w-3.5 h-3.5 rounded-full border-2 flex items-center justify-center ${config.productType === opt.value ? "border-pink-500 bg-pink-500" : "border-gray-300"}`}>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="p-4 bg-purple-50/60 border border-purple-100 rounded-2xl space-y-3">
                      <div className="flex items-center gap-1.5 mb-1">
                        ...
                        <span className="text-xs font-bold text-purple-700 uppercase tracking-wide">Order Quantity Settings</span>
                      </div>
                      ...
                      {(config.minQuantity > 1 || config.quantityStep > 1) && (
                        <p className="text-[11px] text-purple-600 bg-purple-100 px-3 py-1.5 rounded-lg">
                ...
                {config.productType === "custom_print" && (
                  <div className="border border-gray-200 rounded-2xl overflow-hidden">
                    ...
                    <div className="p-5 space-y-5">
                      ...
                      <div className="p-4 bg-purple-50/60 border border-purple-100 rounded-2xl space-y-3 mt-3">
                        <div className="flex items-center gap-1.5 mb-1">
                          <Hash size={12} className="text-purple-500" />
                          <span className="text-xs font-bold text-purple-700 uppercase tracking-wide">Custom Order Quantity Settings</span>
                        </div>
              ...
              <section className="space-y-4 rounded-2xl border border-pink-100 bg-gradient-to-br from-pink-50/80 to-purple-50/80 p-4 sm:p-5">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <h3 className="flex items-center gap-2 text-sm font-bold text-gray-800"><Gift size={16} className="text-pink-500" /> Customer Offer</h3>
                  ...
                  <label className="inline-flex cursor-pointer items-center gap-2 text-xs font-semibold text-gray-700">
                    <input type="checkbox" checked={config.offerEnabled} onChange={e => setC({ offerEnabled: e.target.checked })} className="h-4 w-4 accent-pink-500" /> Enable offer
                  </label>
                </div>
                {config.offerEnabled && (
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-[180px_1fr]">
                    <div>
                      ...
                      <input type="number" min={0} step="0.01" value={config.offerMinAmount || ""} onChange={e => setC({ offerMinAmount: Math.max(0, Number(e.target.value) || 0) })} className="w-full rounded-xl border border-pink-200 bg-white px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-pink-300" placeholder="e.g. 1500" />
                    </div>
                    <div>
                      <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-gray-500">Offer message</label>
                      <textarea value={config.offerMessage} onChange={e => setC({ offerMessage: e.target.value })} rows={2} className="w-full resize-none rounded-xl border border-pink-200 bg-white px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-pink-300" placeholder="e.g. Get a FREE cute sticker pack with this order!" />
                    </div>
                  </div>
                )}
              </section>
              ...
              <div className="pt-2">
                <button
                  ...
                  className="w-full py-3.5 bg-gradient-to-r from-pink-500 to-purple-600 text-white font-bold rounded-2xl disabled:opacity-60 hover:opacity-90 transition-opacity shadow-lg shadow-pink-500/20"
                >
          ...
          <div className="flex min-h-full items-start sm:items-center justify-center p-3 sm:p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl my-4 sm:my-0" style={{ maxHeight: "calc(100vh - 32px)" }}>
              <div className="flex items-center justify-between p-4 sm:p-5 border-b border-gray-100 sticky top-0 bg-white rounded-t-2xl">
                <div>
                  <h2 className="font-bold text-base sm:text-lg text-gray-900 flex items-center gap-2"><Tag size={18} className="text-pink-500" /> Manage Categories</h2>
                  ...
                  {(categories ?? []).length === 0 ? (
                    ...
                    <ul className="divide-y divide-gray-50">
                      {(categories ?? []).map((cat: any) => {
                        ...
                        return (
                          <li key={cat.id} className={`flex items-center justify-between gap-2 px-3 sm:px-4 py-2.5 sm:py-3 ${isEditing ? "bg-pink-50" : "hover:bg-gray-50"}`}>
                            <div className="min-w-0 flex-1">
                              <div className="flex items-center gap-2">
                                <span className="font-semibold text-gray-900 text-sm truncate">{cat.name}</span>
                                <span className="text-[10px] font-bold px-2 py-0.5 bg-pink-50 text-pink-600 rounded-full border border-pink-100 shrink-0">{count}</span>
                              </div>
