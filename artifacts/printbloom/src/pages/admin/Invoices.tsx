    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <Receipt size={22} className="text-pink-500" />
        ...
        <div className="grid grid-cols-2 sm:flex gap-2 sm:gap-2 shrink-0">
          <button
            ...
            className="flex items-center justify-center gap-1.5 px-3 sm:px-4 py-2 rounded-xl bg-gradient-to-r from-pink-500 to-purple-600 text-white text-xs sm:text-sm font-semibold shadow-md shadow-pink-500/25"
          >
            <User size={13} /> <span className="whitespace-nowrap">From Client</span>
          </button>
          <button
            onClick={() => { resetForm(); setEntryMode("manual"); setShowManual(true); }}
            className="flex items-center justify-center gap-1.5 px-3 sm:px-4 py-2 rounded-xl border-2 border-pink-300 text-pink-600 bg-white text-xs sm:text-sm font-semibold hover:bg-pink-50 transition-colors"
          >
      ...
      <div className="rounded-2xl border border-gray-100 bg-white px-4 py-3 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <select value={dateFilter} onChange={e => setDateFilter(e.target.value as InvoiceDateRange)} className="rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-xs sm:text-sm font-bold text-gray-700 outline-none focus:border-pink-400">
        ...
        {dateFilter === "custom" && <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-gray-100 pt-3">
          ...
          <input type="date" value={customFrom} max={customTo || undefined} onChange={e => setCustomFrom(e.target.value)} className="rounded-lg border border-gray-200 px-2.5 py-1.5 text-xs outline-none focus:border-pink-400" />
          <span className="text-[11px] font-bold text-gray-400">TO</span>
          <input type="date" value={customTo} min={customFrom || undefined} onChange={e => setCustomTo(e.target.value)} className="rounded-lg border border-gray-200 px-2.5 py-1.5 text-xs outline-none focus:border-pink-400" />
        </div>}
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 px-3 sm:px-5 py-2.5 sm:py-3 border-b border-gray-100">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <select
              ...
              className="rounded-xl border border-gray-200 bg-white px-3 py-2 text-xs sm:text-sm font-semibold text-gray-700 outline-none focus:border-pink-400"
        ...
        {selectedIds.size > 0 && (
          <div className="px-4 sm:px-5 py-2.5 border-b border-pink-100 bg-pink-50 flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0">
              <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-pink-500 text-white text-[11px] font-bold shrink-0">{selectedIds.size}</span>
              <span className="text-xs sm:text-sm font-semibold text-pink-700 truncate">selected</span>
            </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {filteredInvoices.map(inv => {
                ...
                return (
                  <tr key={inv.id} className={`hover:bg-gray-50/50 ${isSelected ? "bg-pink-50/40" : ""}`}>
                    ...
                    <td className="px-4 py-3 font-mono text-pink-500 text-xs font-bold whitespace-nowrap">{inv.invoiceNumber}</td>
                      ...
                      {isEditing ? (
                        <div className="flex items-center gap-1.5">
                          <select value={editInvStatus} onChange={e => setEditInvStatus(e.target.value)}
                            className="text-xs border border-gray-200 rounded-lg px-2 py-1 outline-none focus:ring-1 focus:ring-pink-300">
        ...
        <div className="md:hidden divide-y divide-gray-50">
          {filteredInvoices.map(inv => {
            ...
            const isSelected = selectedIds.has(inv.id);
            return (
              <div key={inv.id} className={`p-4 space-y-2.5 ${isSelected ? "bg-pink-50/40" : ""}`}>
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-start gap-2.5 min-w-0 flex-1">
                    <input
                      ...
                      checked={isSelected}
                      onChange={() => toggleSelect(inv.id)}
                      className="w-4 h-4 mt-0.5 rounded border-gray-300 text-pink-500 focus:ring-pink-300 cursor-pointer shrink-0"
                      aria-label={`Select invoice ${inv.invoiceNumber}`}
                    />
                    <div className="min-w-0 flex-1">
                      <div className="font-mono text-pink-500 text-[11px] font-bold truncate">{inv.invoiceNumber}</div>
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button onClick={() => openEdit(inv)} aria-label="Edit"
                      className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-500 bg-pink-50 hover:bg-pink-100 hover:text-pink-600 transition-colors">
                      <Pencil size={15} />
                    </button>
            ...
            <div className="flex-1 overflow-y-auto">
              <div className="p-5 space-y-6">
                ...
                <section>
                  <div className="flex items-center justify-between gap-2 mb-3">
                    ...
                    <div className="flex bg-gray-100 rounded-lg p-0.5 text-[11px] font-semibold">
                      <button
                        ...
                        className={`px-2.5 py-1 rounded-md transition-colors ${entryMode === "existing" ? "bg-white text-pink-600 shadow-sm" : "text-gray-500"}`}
                      >Existing</button>
                      <button
                        type="button"
                        onClick={() => { setEntryMode("manual"); clearClient(); }}
                        className={`px-2.5 py-1 rounded-md transition-colors ${entryMode === "manual" ? "bg-white text-pink-600 shadow-sm" : "text-gray-500"}`}
                      >Manual</button>
                    </div>
                  </div>
                  ...
                  {entryMode === "existing" && (
                    <div className="relative mb-3">
                      <label className="text-[10px] font-semibold text-pink-500 mb-1.5 flex items-center gap-1"><User size={10} /> Select Client from Database *</label>
                        ...
                        <div className="absolute top-full mt-1 left-0 right-0 bg-white border border-gray-100 rounded-xl shadow-lg z-10 max-h-56 overflow-y-auto">
                          {filteredClients.length > 0 ? (
                            filteredClients.map((c: any) => (
                              <button key={c.id} type="button" onClick={() => selectClient(c)} className="w-full text-left px-4 py-2.5 hover:bg-pink-50 text-sm border-b border-gray-50 last:border-0">
                          ...
                          {showAddNewClient && (
                            <button
                              ...
                              className="w-full text-left px-4 py-2.5 hover:bg-pink-50 text-sm border-t border-gray-100 flex items-center gap-2 text-pink-600 font-semibold"
                            >
                              <UserPlus size={13} />
                  ...
                  {entryMode === "manual" && (
                    <div className="mb-3">
                      <label className="text-[10px] font-semibold text-pink-500 mb-1.5 flex items-center gap-1"><User size={10} /> Client Full Name *</label>
                      <input
                  ...
                  {(entryMode === "manual" || form.clientName) ? (
                    <div className="space-y-2.5" onClick={() => setShowClientDropdown(false)}>
                      {entryMode === "existing" && form.clientName && (
                        <div className={`flex items-center gap-2 px-3 py-2 rounded-xl border ${form.clientId ? "bg-pink-50 border-pink-100" : "bg-amber-50 border-amber-100"}`}>
                          <User size={13} className={`${form.clientId ? "text-pink-500" : "text-amber-500"} shrink-0`} />
                          <span className={`text-sm font-semibold flex-1 truncate ${form.clientId ? "text-pink-700" : "text-amber-700"}`}>{form.clientName}</span>
                          {form.clientId ? (
                            <span className="text-[10px] font-bold uppercase tracking-wide text-pink-500 bg-white/60 px-1.5 py-0.5 rounded flex items-center gap-1">
                    ...
                    <p className="text-xs text-gray-400 italic">Search and select a client above to fill in their details, or switch to <button type="button" onClick={() => setEntryMode("manual")} className="text-pink-500 font-semibold underline">Manual</button>.</p>
                  )}
                </section>
                ...
                <section>
                  ...
                  <div className="relative mb-3 rounded-2xl border border-purple-100 bg-purple-50/60 p-3">
                    <div className="mb-2 flex items-center justify-between"><div><div className="text-xs font-bold text-purple-700">Add from Products & Services</div><div className="text-[10px] text-gray-400">Search the catalog, or continue with manual line items below.</div></div><span className="rounded-full bg-white px-2 py-1 text-[10px] font-bold text-purple-600">{catalogItems.length} items</span></div>
                    <div className="relative">
                      ...
                      {showCatalogDropdown&&<div className="absolute left-0 right-0 top-full z-30 mt-1 max-h-64 overflow-y-auto rounded-xl border border-gray-200 bg-white p-1 shadow-xl">{filteredCatalogItems.length===0?<div className="px-3 py-6 text-center text-xs text-gray-400">No matching product or service</div>:filteredCatalogItems.map(item=><button key={item.key} type="button" onMouseDown={e=>e.preventDefault()} onClick={()=>addCatalogItem(item)} className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left hover:bg-purple-50"><span className={`rounded-md px-2 py-1 text-[9px] font-black ${item.kind==="Product"?"bg-pink-50 text-pink-600":"bg-blue-50 text-blue-600"}`}>{item.kind}</span><span className="min-w-0 flex-1"><span className="block truncate text-xs font-bold text-gray-800">{item.name}</span>{item.category&&<span className="block text-[10px] text-gray-400">{item.category}</span>}</span><span className="text-xs font-black text-purple-600">{num(item.price)>0?rs(item.price):"Manual price"}</span></button>)}</div>}
                    </div>
                  </div>
