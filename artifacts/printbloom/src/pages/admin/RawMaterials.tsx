...
const CATEGORIES_LIST = ["Paper", "Ink", "Lamination", "Vinyl", "Finishing", "Packaging", "Other"];
...
export default function RawMaterials() {
  ...
  const handleAdjust = (e: React.FormEvent) => {
    ...
    adjustStock({ id: adjustItem.id, data: { quantity: newQty } });
  };
      ...
      <div className="flex items-start sm:items-center justify-between gap-3 flex-wrap">
        <div>
          <div className="flex items-center gap-2">
            <FlaskConical size={22} className="text-pink-500" />
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button onClick={() => { setEditing(null); setForm(EMPTY); setShowForm(true); }} className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-gradient-to-r from-pink-500 to-purple-600 text-white text-sm font-semibold shadow-sm">
      ...
      <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
        ...
        {filtered.length === 0 ? (
          <div className="py-20 text-center text-gray-300">
            ...
            <button onClick={() => { setEditing(null); setForm(EMPTY); setShowForm(true); }} className="mt-5 flex items-center gap-1.5 px-4 py-2 rounded-lg bg-gradient-to-r from-pink-500 to-purple-600 text-white text-sm font-semibold mx-auto">
            ...
            <form onSubmit={handleAdjust} className="p-5 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-pink-200"
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              <div>
                ...
                <input required value={form.name} onChange={e => setForm((f: any) => ({ ...f, name: e.target.value }))} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-pink-200" placeholder="A4 Bond Paper 80gsm" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-gray-400 block mb-1">Quantity</label>
                  <input type="number" value={form.quantity} onChange={e => setForm((f: any) => ({ ...f, quantity: e.target.value }))} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-pink-200" />
                </div>
                <div>
                  <label className="text-xs text-gray-400 block mb-1">Unit</label>
                  <input value={form.unit} onChange={e => setForm((f: any) => ({ ...f, unit: e.target.value }))} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-pink-200" placeholder="reams, rolls, liters" />
                </div>
                <div>
                  <label className="text-xs text-gray-400 block mb-1">Low Stock Alert At</label>
                  <input type="number" value={form.lowStockThreshold} onChange={e => setForm((f: any) => ({ ...f, lowStockThreshold: e.target.value }))} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-pink-200" />
                </div>
                <div>
                  <label className="text-xs text-gray-400 block mb-1">Cost per Unit (Rs.)</label>
                  <input type="number" value={form.cost} onChange={e => setForm((f: any) => ({ ...f, cost: e.target.value }))} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-pink-200" placeholder="0" />
                </div>
              </div>
              <div>
                <label className="text-xs text-gray-400 block mb-1">Supplier</label>
                <input value={form.supplier} onChange={e => setForm((f: any) => ({ ...f, supplier: e.target.value }))} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-pink-200" placeholder="Supplier name" />
              </div>
              <div>
              ...
              <button type="submit" disabled={creating || updating} className="w-full py-2.5 bg-gradient-to-r from-pink-500 to-purple-600 text-white text-sm font-bold rounded-xl disabled:opacity-60">
